...
export default function CardPage() {
  ...
  const selectedCardType = useSelector((state: RootState) => state.phoneCard.selected)
  ...
  return (
    <PageWrapper>
      ...
      <div className='px-10'>
        <Input className='h-10' value={phone} onChange={(e: any) => { setPhone(e.target.value) }} allowClear />
      </div>
      ...
      <div className='mt-5 flex items-center justify-center w-full flex-wrap  '>
        {telcoImages?.map((e: string, i: number) => (
          ...
          }} key={i} className={`${selected == e ? "bg-[#f5f5f5] border border-m_red" : ''} w-[261px] h-[80px] border flex items-center justify-center mr-3 mb-3 rounded-lg`}>
            {getImageTelco(e)}
          </Button>
        ))}
      </div>
      ...
      <Button
        ...
        loading={loading} className='select-none ml-auto active:opacity-70 bg-m_red text-white rounded-lg px-6 py-2 flex justify-center items-center m-auto mt-10 w-[177px] h-[48px]'>
