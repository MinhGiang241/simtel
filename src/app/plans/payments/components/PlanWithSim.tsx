...
import { setSeleted, setTelco } from "@/GlobalRedux/SimPack/SimPackSlice";
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        var dataSubmit: Order = {
          ...
          total_amount: (number?.price ?? 0) + (transportFee ?? 0),
          prod_total_amount: number?.price ?? 0,
        ...
        createOrder(dataSubmit)
          .then(async (v) => {
            ...
            pushPathName(router, dispatch, `/plans/payments?order=${v}`);
            if (method === "Wallet" && number) {
              ...
              pushPathName(router, dispatch, `/pay?order=${v}&type=simandpack`);
            }
          })
          .then((v) => {
            if (v?.paymentUrl) {
        ...
        .then(async (v) => {
          ...
          if (v) {
            ...
            getSimPackById(v.i?.simPackId).then((value) => {
              if (value) {
                dispatch(setSeleted(value));
                dispatch(setTelco(value.telco));
              }
            });
          }
        })
            ...
            <>
              ...
              <div className="flex w-full justify-between my-4">
                ...
                <p className="font-bold text-base">
                  {number?.type === "Physical" ? "Sim vật lý" : "eSIM"}
                </p>
              </div>
            </>
          )}
        ...
        <div className="flex justify-between mx-5 mb-8 items-center">
          <div className="flex flex-col">
            ...
            <div className="text-2xl font-bold">
              <FormattedNumber
                value={number ? (number.price ?? 0) + (transportFee ?? 0) : 0}
                style="currency"
                currency="VND"
              />
            </div>
