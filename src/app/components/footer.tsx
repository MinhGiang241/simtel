      ...
      <div className='footer px-10 lg:max-w-[1140px] m-auto' >
        <div className="lg:grid lg:grid-cols-4 lg:gap-4 flex flex-wrap ">
          ...
          <div className="col-span-1 p-4">
            ...
            <div className='text-base font-normal pb-2 text-slate-500'>Email: simtel.vn@gmail.com</div>
          </div>
          <div className="col-span-1 p-4">
            <div className='font-bold text-xl pb-3 text-m_text_footer'>Chính sách</div>
            {article?.map((e, i) => (<div key={i} className='text-base font-normal pb-2 text-slate-500 cursor-pointer' onClick={() => { pushPathName(router, dispatch, `/resolution?id=${e?.article?._id}`) }}>{e?.article?.title}</div>))}
          </div>
          {/* <div className="col-span-1 p-4">
          ...
          <div className="col-span-1 p-4">
            <div className='font-bold text-xl pb-3 text-m_text_footer'>Danh mục</div>
            <div className='text-base font-normal pb-2 text-slate-500 cursor-pointer' onClick={() => { pushPathName(router, dispatch, '/plans') }}>Gói cước</div>
            <div className='text-base font-normal pb-2 text-slate-500 cursor-pointer' onClick={() => { pushPathName(router, dispatch, '/sims') }}>Mua sim</div>
            <div className='text-base font-normal pb-2 text-slate-500 cursor-pointer' onClick={() => { pushPathName(router, dispatch, '/cards') }}>Nạp thẻ</div>
            <div className='text-base font-normal pb-2 text-slate-500 cursor-pointer' onClick={() => { pushPathName(router, dispatch, '/blog') }}>Khuyến mại</div>
          </div>
        </div>
      </div>
      <div className='bg-m_red h-[72px] w-full flex justify-center items-center'>
        <div className='w-[1140px] lg:flex justify-between items-center'>
          <div className='flex w-[120px] h-[24px] mb-2'>
            <Fb_footer className="mr-2" />
            <Twitter_footer className="mr-2" />
            <Mess_footer className="mr-2" />
          </div>
          <div className='text-white text-sm font-normal'>@Copyright, Masflex 2023. All rights reserved.</div>
        </div>
      </div>
    </div>
  )
